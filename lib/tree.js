...
function tree(schema, options) {
            ...
            ref: {
                ...
                set: function (val) {
                    if (typeof(val) === "object" && val._id) {
                        this.parent.id = val.id;
                        this.parent.name = val.name;
                        return val._id;
                    }
                    return val;
                }
    ...
    schema.pre('save', function (next) {
        var isParentChange = this.isModified('parent.ref');
        ...
        if (this.isNew || isParentChange) {
            ...
            var self = this;
            this.collection.findOne({ _id: this.parent.ref }, function (err, doc) {
    ...
    schema.method('getChildren', function (recursive, cb) {
        ...
        var filter = recursive ? { path: { $regex: '^' + this.path + pathSeparator } } : { 'parent.ref' : this._id };
        return this.model(this.constructor.modelName).find(filter, cb);
    });

    schema.method('getParent', function (cb) {
        return this.model(this.constructor.modelName).findOne({ _id: this.parent.ref }, cb);
    });
